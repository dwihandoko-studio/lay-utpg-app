...
class Upload extends BaseController
{
        ...
        foreach ($lists as $list) {
            ...
            if ($list->lock_upload_spj == 1) {
                if ($list->lampiran_pernyataan == NULL || $list->lampiran_pernyataan == "") {
                    $row[] = '<a href="javascript:showNotifUpload(this);"><span class="badge rounded-pill badge-warning">Belum Upload</span></a>';
                } else {
                ...
                // $row[] = '<a target="popup" onclick="window.open(\'' . base_url('upload/spj/tamsil') . '/' . $list->lampiran_sk_dirgen . '\',\'popup\',\'width=600,height=600\'); return false;" href="' . base_url('upload/spj/tamsil') . '/' . $list->lampiran_sk_dirgen . '"><span class="badge rounded-pill badge-soft-dark">Lihat</span></a>';
            } else {
                if ($list->lampiran_pernyataan == null || $list->lampiran_pernyataan == "") {
                ...
                // } else {
                //     // $row[] = '<a target="popup" onclick="window.open(\'' . base_url('upload/spj/tamsil') . '/' . $list->lampiran_sk_dirgen . '\',\'popup\',\'width=600,height=600\'); return false;" href="' . base_url('upload/spj/tamsil') . '/' . $list->lampiran_sk_dirgen . '"><span class="badge rounded-pill badge-soft-dark">Lihat</span></a>';
                //     $row[] = '<a target="popup" onclick="window.open(\'' . base_url('upload/spj/tamsil') . '/' . $list->lampiran_sk_dirgen . '\',\'popup\',\'width=600,height=600\'); return false;" href="' . base_url('upload/spj/tamsil') . '/' . $list->lampiran_sk_dirgen . '"><button type="button" class="btn btn-primary btn-sm btn-rounded waves-effect waves-light mr-2 mb-1">
                //                 <i class="bx bxs-show font-size-16 align-middle"></i></button>
                //             </a>
            ...
            switch ((int)$list->status_usulan) {
                ...
                default:
                    if ($list->lampiran_pernyataan == NULL || $list->lampiran_pernyataan == "" || $list->lampiran_rekening_koran == null || $list->lampiran_rekening_koran == "") {
    ...
    public function aksiajukanulang()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($this->_db->affectedRows() > 0) {
                createAktifitas($user->data->id, "Mengajukan ulang laporan SPJ TAMSIL", "Mengajukan Ulang SPJ TAMSIL", "edit");
                $this->_db->transCommit();

                $response = new \stdClass;
                $response->status = 200;
                $response->message = "Laporan SPJ TAMSIL Tahun $tahun Tw. $triwulan berhasil diajukan ulang.";
                return json_encode($response);
            } else {
