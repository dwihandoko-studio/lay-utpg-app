...
use App\Models\Situgu\Ks\Us\Tpg\TerbitskModel;
use Config\Services;
...
class Skterbit extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $idPtk = $this->_helpLib->getPtkId($userId);
        $lists = $datamodel->get_datatables($idPtk);
        $data = [];
        ...
        $output = [
            "draw" => $request->getPost('draw'),
            "recordsTotal" => $datamodel->count_all($idPtk),
            "recordsFiltered" => $datamodel->count_filtered($idPtk),
            "data" => $data
        ];
        echo json_encode($output);
    }
    ...
    {
        return redirect()->to(base_url('situgu/ks/us/tpg/skterbit/data'));
    }
    ...
    public function data()
    {
        ...
        return view('situgu/ks/us/tpg/skterbit/index', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->data = view('situgu/ks/us/tpg/detail-lolos', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
