...
class Upload extends BaseController
{
    ...
    public function getAll()
    {
        ...
        foreach ($lists as $list) {
            ...
            //     </a>
            //     < href="javascript:actionHapus(\'' . $list->id . '\', \'' . str_replace("'", "", $list->nama)  . '\', \'' . $list->nuptk . '\');" class="delete" id="delete"><button type="button" class="btn btn-danger btn-sm btn-rounded waves-effect waves-light mr-2 mb-1">
    ...
    public function formupload()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/ptk/spj/tpg/upload/upload', $data);
            return json_encode($response);
        }
    }
    ...
    public function editformupload()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/ptk/spj/tpg/upload/editupload', $data);
            return json_encode($response);
        }
    }
